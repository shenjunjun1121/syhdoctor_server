...
public abstract class AnswerBaseService extends BaseService {
    ...
    public void addAnswerOrderPushData(String orderNo) {
        ...
        systemService.addMessage("", TextFixed.messageOrderTitle,
                ...
                TypeNameAppPushEnum.answerOrderDetail.getCode(), userId, String.format(TextFixed.answerUserOrderPushText, UnixUtil.getDate(createTime, "yyyy-MM-dd HH:mm:ss")), "");//app 用户内推送
        //用户端消息结束
        ...
        systemService.addMessage("", TextFixed.messageOrderTitle,
                ...
                TypeNameAppPushEnum.answerOrderDetail.getCode(), doctorId, String.format(TextFixed.answerDoctorOrderText, name), "");//app 医生 内推送
        //医生端消息结束
    ...
    public void addPhoneOrderPushData(String orderNo) {
        ...
        systemService.addMessage("", TextFixed.messageOrderTitle,
                MessageTypeEnum.doctor.getCode(), String.valueOf(orderId),
                TypeNameAppPushEnum.phoneOrderDetail.getCode(), doctorId, doctorText, "");//app 医生 内推送
        //医生端消息结束
    }
    ...
    public void addDoctorAnswerPushData(long orderId, int contentType, int questionaAnswerType) {
        ...
        if (questionaAnswerType == 0 && doctoronline != 1) {//用户回复,给医生提醒
            ...
            systemService.addMessage(userUrl, TextFixed.messageServiceTitle,
                    MessageTypeEnum.doctor.getCode(), String.valueOf(orderId),
            ...
            systemService.addMessage(doctorUrl, TextFixed.messageServiceTitle,
                    MessageTypeEnum.user.getCode(), String.valueOf(orderId),
     ...
     * @return
     */
    public Map<String, Object> getDoctorAnswerList(long orderid, int pageindex, int pagesize) {
    ...
    private void getIdsByDoctor
        ...
        for (Map<String, Object> map : answerList) {
            ...
            if (id > 0) {
                ...
                } else if (QAContentTypeEnum.Prescription.getCode() == contenttype) {
     ...
     * @return
     */
    public long addAppAnswerPrescriptionExamine(long orderId, long doctorid, String diagnosis, List<?> druglist) {
        return prescriptionService.sendPrescriptionExamine(orderId, doctorid, diagnosis, druglist, OrderTypeEnum.Answer.getCode());
    }
...
//        String result = getSoundRecording("u0KguVuZvAzql9z8r7ViPb93Te8hqPIr29N9KGgjf3E2fnVKbEwLgynbCDbXDeLm", 406, 1000);
    ...
    public Map<String, Object> sendPrescription(long orderId, String presNo, long oftenPrescriptionId) {
        ...
        if (problem != null) {
            ...
            if (states == 2 || states == 6) {
                ...
                long doctorid = ModelUtil.getLong(problem, "doctorid");
                addDoctorAnswerPushData(orderId, QAContentTypeEnum.Prescription.getCode(), 1);
                long answerId = answerMapper.addAnswer(userId, doctorid, orderId, String.valueOf(prescriptionid), QAContentTypeEnum.Prescription.getCode(), 1);
                answerMapper.updateAnswer(orderId);
                //websocket
                sendSocket(answerId);

                map = answerMapper.getDoctorAnswer(answerId);
                if (ModelUtil.getInt(map, "contenttype") == 4) {
    ...
    public void answerOrderSuccessFailPushData(String orderNo, TypeNameAppPushEnum typeNameAppPushEnum) {
        ...
        systemService.addMessage("", TextFixed.messageServiceTitle,
                MessageTypeEnum.user.getCode(), String.valueOf(orderId),
                typeNameAppPushEnum.getCode(), userId, userText, "");//app 用户内推送
        //用户端消息结束
    ...
    public static void main(String[] args) {
        Map<String, Object> params = new HashMap<>();
        params.put("user_id", "97260ab0dc2a11e8a509f57c32adf22f");
        params.put("jumptype", 3);
        params.put("pay_amount", String.valueOf(10000000));
        params.put("order_id", "156156");
        String s = JsonUtil.getInstance().gsonToJson(params);
        Map<String, Object> post = HttpUtil.getInstance().post(JumpLink.KANGYANG_PAY_URL, s);
     ...
     * @return
     */
    public List<Map<String, Object>> getDoctorInquiryListExport(long begintime, long endtime) {
        List<Map<String, Object>> list = answerMapper.getDoctorInquiryListExport(begintime, endtime);
        Map<String, Object> map = new HashMap<>();
        map.put("doctorid", "id");
    ...
    public List<Map<String, Object>> getDoctorInquiryList(long begintime, long endtime, int pageIndex, int pageSize) {
        return answerMapper.getDoctorInquiryList(begintime, endtime, pageIndex, pageSize);
    }

    public long getDoctorInquiryListCount(long begintime, long endtime) {
        return answerMapper.getDoctorInquiryListCount(begintime, endtime);
    }
