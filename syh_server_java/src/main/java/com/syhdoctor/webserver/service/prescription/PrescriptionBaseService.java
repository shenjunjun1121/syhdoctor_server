...
public abstract class PrescriptionBaseService extends BaseService {
    ...
    public long addPrescriptionByOften(long oftenPrescriptionId, String presNo, long orderId, int orderType) {
        ...
        long reviewTime = UnixUtil.getNowTimeStamp();
        // 审核医生编码 review_doc_code
        String reviewDocCode = ModelUtil.getStr(reviewDoctor, "indoccode");
        // 审核医生姓名 review_doc_name
        String reviewDocName = ModelUtil.getStr(reviewDoctor, "docname");
        // 审方时间 trial_time
        long trialTime = UnixUtil.getNowTimeStamp();
        // 审方医生编码 trial_doc_code
        String trialDocCode = ModelUtil.getStr(trialDoctor, "indoccode");
        ...
        long prescriptionid = prescriptionMapper.addPrescription(presNo, diagnosis, userId, presClassCode, presClassName, medRdNo, medClassCode, medClassName, ptNo, geCode, geName,
                ...
                mobilityFlag, longMedicalFlag, presEffecDays, totalPrice, orderId, orderType, 2);
        // 药品列表 drug_list
...
//        String keyPath = "/home/qwq/桌面/111111111.jpg";
...
//        int no = fm.stringWidth(String.format("处方编号：%s", "201810121510358308192"));
//        int time = fm.stringWidth(String.format("有效时间%s至%s", "2018-10-30", "2018-10-31"));
//        int a = width - time - 50;
//        int b = webh019 + 50;
//        int c = (a - b) / 2;
//        int d = (b + c) - no / 2;
...
//        g.drawString(String.format("处方编号：%s", "201810121510358308192"), d, index);
//        g.drawString(String.format("有效时间%s至%s", "2018-10-30", "2018-10-31"), width - fm.stringWidth(String.format("有效时间%s至%s", "2018-10-30", "2018-10-31")) - 50, index);
//        g.setFont(font1);
...
//        g.drawString(getUnLineText("邱文强", font).getIterator(), fm.stringWidth("姓名：") + 100, index);
//
//        g.drawString("性别：", 500, index);
//        g.drawString(getUnLineText("男", font).getIterator(), fm.stringWidth("性别：") + 500, index);
//
//        g.drawString("年龄：", 900, index);
//        g.drawString(getUnLineText("24岁", font).getIterator(), fm.stringWidth("年龄：") + 900, index);
//
//        index = index + 50;
//        g.drawString("体重：", 100, index);
//        g.drawString(getUnLineText("75kg", font).getIterator(), fm.stringWidth("体重：") + 100, index);
//
//        g.drawString("身高：", 500, index);
//        g.drawString(getUnLineText("180cm", font).getIterator(), fm.stringWidth("身高：") + 500, index);
//
//        g.drawString("身份证号：", 900, index);
//        g.drawString(getUnLineText("362324199401213333", font).getIterator(), fm.stringWidth("身份证号：") + 900, index);
//
//        index = index + 50;
//        g.drawString("过往病史：", 100, index);
//        g.drawString(getUnLineText("高血压、两年", font).getIterator(), fm.stringWidth("过往病史：") + 100, index);
//
//        index = index + 50;
//        g.drawString("诊断结果：", 100, index);
//        g.drawString(getUnLineText("高血压", font).getIterator(), fm.stringWidth("诊断结果：") + 100, index);
//
//        index = index + 50;
//
//        g.drawString("-------------------------------------------------------------------------------", 100, index);
//
//        index = index + 50;
//        g.setFont(font2);
...
//        g.setFont(font);
//        for (int i = 1; i < 2 + 1; i++) {
//            g.drawString(String.format("%s、(%s 规格：%s)", i, "阿莫西林", "0.75g*20片"), 100, index + 50);// 画出字符串
//            g.drawString(String.format("*%s", "3"), fm.stringWidth(String.format("%s、(%s 规格：%s)", i, "阿莫西林", "0.75g*20片")) + 200, index + 50);// 画出字符串
//            String usagemethod = String.format("%s,每次%s%s%s", "每天三次", "一片", "", "");
...
//        try {
//            //保持纵横比，质量降低为原来的42%
//            String th = "/home/qwq/桌面/4444.png";
//            String hos = "/home/qwq/桌面/5555.png";
//            Thumbnails.of("/home/qwq/IdeaProjects/syhdoctor_server/syh_server_java/build/libs/small_doctor_url.png")
//                    .size(293, 176)
//                    .toFile(th);
//
//            Thumbnails.of("/home/qwq/桌面/small_doctor_url.png")
//                    .size(240, 240)
//                    .toFile(hos);
//
//            String x2 = zipImageFile(th, 293, 176, 1f, "x2");
//
//            String newhos = zipImageFile(hos, 240, 240, 1f, "x2");
//
//
//            FontMetrics finalFm = fm;
//            Thumbnails.of(outFile)
//                    .size(width, height)
//                    .watermark((int enclosingWidth, int enclosingHeight, int width1, int height1, int insetLeft, int insetRight, int insetTop, int insetBottom) -> new Point(finalFm.stringWidth("医师：") + 100, height - 240), ImageIO.read(new File(th)), 1f)
//                    .outputQuality(1f).toFile(keyPath);
//            Thumbnails.of(outFile)
//                    .size(width, height)
//                    .watermark((int enclosingWidth, int enclosingHeight, int width1, int height1, int insetLeft, int insetRight, int insetTop, int insetBottom) -> new Point(finalFm.stringWidth("药师审方：") + 500, height - 240), ImageIO.read(new File(th)), 1f)
//                    .outputQuality(1f).toFile(keyPath);
//            Thumbnails.of(outFile)
//                    .size(width, height)
//                    .watermark((int enclosingWidth, int enclosingHeight, int width1, int height1, int insetLeft, int insetRight, int insetTop, int insetBottom) -> new Point(finalFm.stringWidth("药师复核：") + 900 + finalFm.stringWidth("药师"), height - 240), ImageIO.read(new File(th)), 1f)
//                    .outputQuality(1f).toFile(keyPath);
//
//            Thumbnails.of(outFile)
//                    .size(width, height)
//                    .watermark((int enclosingWidth, int enclosingHeight, int width1, int height1, int insetLeft, int insetRight, int insetTop, int insetBottom) -> new Point(width - 430, height - 400), ImageIO.read(new File(hos)), 1f)
//                    .outputQuality(1f).toFile(keyPath);
//
//        } catch (IOException e) {
//            e.printStackTrace();
...
//            out.close();
//            ImageIO.write(oldFile,filePrex , new File(newImage));
...
//            String medicineFreqName = ModelUtil.getStr(map, "medicinefreqname");
        ...
        try {
            ...
            if (!StrUtil.isEmpty(reviewDoctorUrl, trialDoctorUrl)) {
                Thumbnails.of(outFile)
                        .size(width, height)
                        .watermark((int enclosingWidth, int enclosingHeight, int width1, int height1, int insetLeft, int insetRight, int insetTop, int insetBottom) -> new Point(width - 430, height - 600), ImageIO.read(new File(hos)), 1f)
                        .outputQuality(1f).toFile(keyPath);
...
//            }
    ...
    //自动审核
    public long sendPrescription(long orderid, long doctorid, String diagnosis, List<?> druglist, int orderType) {
        Map<String, Object> map = previewSave(orderid, doctorid, diagnosis, druglist);
        String presNo = ModelUtil.getStr(map, "preno");
        String presPhoto = ModelUtil.getStr(map, "presphoto");
        String photoUrl = ModelUtil.getStr(map, "photourl");
        Long prescriptionId = savePrestion(orderid, doctorid, diagnosis, druglist, presNo, presPhoto, photoUrl, orderType);
        sendCenter(prescriptionId);//上传到数据中心
        return prescriptionId;
    }
    ...
    public long sendPrescriptionExamine(long orderid, long doctorid, String diagnosis, List<?> druglist, int orderType) {
        ...
        String presNo = ModelUtil.getStr(map, "preno");
        String photoUrl = ModelUtil.getStr(map, "photourl");
        Long prescriptionId = savePrestionExamine(orderid, doctorid, diagnosis, druglist, presNo, photoUrl, orderType);
        return prescriptionId;
    }

    //后台审核
    public long prescriptionExamine(long prescriptionId, int examine, String remark) {
        ...
        if (examine == examine1) {
            throw new ServiceException("不能重复审核");
        }

        long time = 0;
        String reviewIndoccode = null;
        String reviewDocname = null;
        String trialIndoccode = null;
        ...
        if (examine == 2) {
            ...
            String presphotourl = ConfigModel.BASEFILEPATH + FileUtil.getFileName(ModelUtil.getStr(prescription, "presphotourl"));
            ...
            trialIndoccode = ModelUtil.getStr(trialDoctor, "indoccode");
            trialDocname = ModelUtil.getStr(trialDoctor, "docname");
            String trialDoctorUrl = ConfigModel.BASEFILEPATH + FileUtil.getFileName(ModelUtil.getStr(trialDoctor, "digitalsignurl"));

            time = UnixUtil.getNowTimeStamp();

            createImage3(presphotourl, reviewDoctorUrl, trialDoctorUrl);
        ...
        prescriptionMapper.updatePrescriptionExamine(prescriptionId, presPhoto, examine, remark, time, reviewIndoccode, reviewDocname, time, trialIndoccode, trialDocname);
    ...
    //发送处方不需要审核
    public Long savePrestion(long orderId, long doctorId, String diagnosis, List<?> druglist, String presNo, String presPhoto, String photoUrl, int orderType) {
        Map<String, Object> problem = answerMapper.getProblem(orderId);
        ...
        doctorId = ModelUtil.getLong(problem, "doctorid");
...
//            String totalCharge = ModelUtil.getStr(map, "totalcharge");
    ...
    //发送处方需要审核
    public Long savePrestionExamine(long orderId, long doctorId, String diagnosis, List<?> druglist, String presNo, String photoUrl, int orderType) {
        Map<String, Object> problem = answerMapper.getProblem(orderId);
        ...
        long prescriptionId = prescriptionMapper.addPrescription(presNo, diagnosis, userId, presClassCode, presClassName, medRdNo, medClassCode, medClassName, ptNo, geCode, geName,
                birthday, ptAge, idNo, ptTel, ptDistrict, insClassCode, insClassName, orgCode, orgName, departmentcode, departmentName, presTime, doctorId, presDocCode, presDocName,
                presDocPhoteData, 0, null, null, 0, null, null, diagCodeType, diseasesCode, diseasesName, diseasesType,
                mobilityFlag, longMedicalFlag, presEffecDays, totalPrice, orderId, orderType, 1);
        // 药品列表 drug_list
        if (druglist.size() == 0) {
            throw new ServiceException("药品不能为空");
        }
...
//            String totalCharge = ModelUtil.getStr(map, "totalcharge");
        ...
        prescriptionMapper.setPrescriptionDocPhoto(null, photoUrl, prescriptionId);
        return prescriptionId;
    }
