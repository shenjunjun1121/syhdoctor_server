...
public abstract class PrescriptionBaseMapper extends BaseMapper {
     ...
     * @param orgCode               医疗机构编码
     * @param orgName               医疗机构名称
     * @param departmentId          就诊科室编码 开方科室编码
    ...
    public long addPrescription(String presNo, String diagnosis, long userId, String prescriptionTypeIds, String prescriptionTypeNames, String medRdNo, String medClassCode, String medClassName,
                                ...
                                String diseasesCode, String diseasesName, String diseasesType, String mobilityFlag, String longMedicalFlag, int presEffecDays, BigDecimal totalPrice, long orderId, int orderType, int examine) {
    ...
    public List<Map<String, Object>> getPrescriptionDrugList(long prescriptionId) {
        String sql = " select prescriptionid, " +
                ...
                "       method, " +
                "       total_charge       totalcharge " +
                "from doc_drug_list where prescriptionid=? ";
        return queryForList(sql, prescriptionId);
    }
    ...
    public List<Map<String, Object>> getDoctorUserPrescriptionList(long doctorId, long userId) {
        String sql = " select middle.prescriptionid, " +
                ...
                "      from doc_prescription dp " +
                "             join doc_drug_list ofd on dp.prescriptionid = ofd.prescriptionid " +
                "      where dp.doctorid = ? " +
                "        and pt_id = ? " +
                "      group by dp.prescriptionid) middle " +
                "       join doc_drug_list dofd on middle.drugsid = dofd.id ";
        List<Object> params = new ArrayList<>();
        params.add(doctorId);
        params.add(userId);
    ...
    public List<Map<String, Object>> getUserPrescriptionList(long userId, int pageIndex, int pageSize) {
        String sql = "  select middle.prescriptionid, " +
                ...
                "      from doc_prescription dp " +
                "             left join doc_drug_list ofd on dp.prescriptionid = ofd.prescriptionid " +
                "      where pt_id=?  " +
                "      group by dp.prescriptionid) middle " +
                "       left join doc_drug_list dofd on middle.drugsid = dofd.id ";
        List<Object> params = new ArrayList<>();
        params.add(userId);
    ...
    public boolean updatePrescriptionExamine(long prescriptionId, String presPhoto, int examine, String remark, long reviewTime, String reviewDocCode, String reviewDocName, long trialTime, String trialDocCode, String trialDocName) {
        String sql = " update doc_prescription set pres_photo=?, examine=?, remark=?,review_time=?,review_doc_code=?,review_doc_name=?,trial_time=?,trial_doc_code=?,trial_doc_name=? where prescriptionid=? ";
        List<Object> params = new ArrayList<>();
        params.add(presPhoto);
        params.add(examine);
        params.add(remark);
