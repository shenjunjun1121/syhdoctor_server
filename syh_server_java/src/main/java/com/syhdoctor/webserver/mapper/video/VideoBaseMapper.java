...
public abstract class VideoBaseMapper extends BaseMapper {
    ...
    public List<Map<String, Object>> findDoctorSchdue(long time) {
    ...
    public List<Map<String, Object>> findDoctorSchduePhone(long time) {
...
//    }
    ...
    public Map<String, Object> getPrescription(long orderid, int ordertype) {
        String sql = " select pres_photo_url presphotourl  from doc_prescription where orderid =? and ifnull(order_type,1)=? and ifnull(examine,2)= 2 order by create_time desc limit 1 ";
        List<Object> list = new ArrayList<>();
        list.add(orderid);
        list.add(ordertype);
