...
//            sql += " and  cdt.id= ? ";
    ...
    public List<Map<String, Object>> getUserCurrentAnswerList(long orderid, int pageindex, int pagesize) {
        String sql = " select ua.headpic     userheadpic, " +
                ...
                "       left join doctor_info di on di.doctorid = da.doctorid " +
                "       left join doctor_problem_order dpo on da.orderid=dpo.id " +
                "where ifnull(da.delflag, 0) = 0  and da.orderid= ? and dpo.states in(2,4,5,6,8) and da.contenttype not in(6,7,10,13,15)";
        List<Object> params = new ArrayList<>();
        params.add(orderid);
        return queryForList(pageSql(sql, " order by da.create_time desc,da.id desc "), pageParams(params, pageindex, pagesize));
    }
    ...
    public List<Map<String, Object>> getUserHistoryAnswerList(long doctorId, long userId, int pageindex, int pagesize) {
        String sql = " select ua.headpic     userheadpic,   " +
                ...
                "            left join doctor_info di on di.doctorid = da.doctorid   " +
                "            left join doctor_problem_order dpo on dpo.id= da.orderid " +
                "     where ifnull(da.delflag, 0) = 0  and da.doctorid= ? and da.contenttype not in(6,7,10,13,15) and da.userid=? and dpo.states in(2,4,5,6,8) ";
        List<Object> params = new ArrayList<>();
        params.add(doctorId);
        params.add(userId);
        return queryForList(pageSql(sql, " order by da.create_time desc,da.id desc "), pageParams(params, pageindex, pagesize));
    ...
    public List<Map<String, Object>> getAppendUserSocketAnswerList(long orderId, long id) {
        String sql = " select ua.headpic     userheadpic,   " +
                ...
                "            left join doctor_info di on di.doctorid = da.doctorid   " +
                "            left join doctor_problem_order dpo on dpo.id= da.orderid " +
                "     where ifnull(da.delflag, 0) = 0  and da.orderid=? and da.contenttype not in(6,7,10,13,15) and dpo.states in(2,4,6) and da.id = ? ";
        List<Object> params = new ArrayList<>();
        params.add(orderId);
        params.add(id);
        return queryForList(sql, params);
    ...
    public List<Map<String, Object>> prescriptionList(List<Long> ids) {
        String sql = " select da.id,prescriptionid,pres_photo_url presphotourl from doctor_answer da left join doc_prescription dp on da.content=dp.prescriptionid where da.id in (:ids) and ifnull(da.delflag, 0) = 0  ";
        Map<String, Object> params = new HashMap<>();
        params.put("ids", ids);
        return queryForList(sql, params);
    }
...
//        }
        ...
        return queryForList(pageSql(sql, " order by din.doctorid,din.visiting_start_time desc "), pageParams(params, pageIndex, pageSize));
    }
